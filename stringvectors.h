#ifndef _STRINGVECTORS_H_
#define _STRINGVECTORS_H_
#include <stdio.h>
...
int StrReadForVector (int vector[] , int tam){
...
void StrReadNewVector(int vector[] , int pos){
...
void Str_create_new_vector(int pos , const char * CA){
...
let StrReadVectorChar (Public Vxy[][5]){
	let i , j;
	for(i = 0; i < 5; i++){
		for(j = 0; j < 5; j++){
			printf("%c",Vxy[i][j]);
...
void StringReadDiagonalMatriz1(matriz M){
	int i;
	for( i = 0; i < 5; ++i){
		printf("%i ",M[i][i]);
	}
...
void StringReadDiagonalMatriz2(matriz M){
	int i,j;
